...
import { QRScanner, type QRScanCallback } from '../../lib/qr-tools/scanner';
import { decodeQRFromImage } from '../../lib/qr-tools/decodeImage';
...
export function QRScannerComponent({ locale = 'en' }: Props) {
  ...
  const elementId = 'qr-scanner-preview';
  const handleScanRef = useRef<QRScanCallback | null>(null);
  ...
  const handleScan: QRScanCallback = useCallback((decodedText) => {
  ...
  const startScanning = async () => {
    ...
    try {
      ...
      setIsScanning(true);
      await scanner.start({ fps: 10, qrbox: { width: 250, height: 250 } }, (text) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('[QRScanner] file upload decode failed:', err);
      setError(
        locale === 'es'
  ...
  return (
    <div className="flex flex-col gap-4">
      <div
        id={elementId}
        className="border border-border bg-surface min-h-[300px] flex items-center justify-center relative overflow-hidden"
      >
        {!isScanning && !isInitializing && (
          <p className="font-mono text-xs text-text-soft px-4 text-center">{t.qrScanPreview}</p>
        )}
          ...
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10">
            <div className="bg-accent/90 text-white px-3 py-1.5 rounded-full flex items-center gap-2">
              ...
              <span className="font-mono text-xs">
                {locale === 'es' ? 'Apunta al código QR' : 'Point at QR code'}
              </span>
            </div>
          </div>
        )}
